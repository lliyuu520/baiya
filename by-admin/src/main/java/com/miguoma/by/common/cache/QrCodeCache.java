...
public class QrCodeCache {
    ...
    public void init() {
        ...
        while (localDate.isBefore(endDateTime)) {
            ...
            final String encodePrefix = WebBase62.encode(Long.parseLong(dayFormat));
            String key = StrUtil.format(QR_CODE_CACHE, dayFormat);
            ...
            log.info("初始化二维码缓存{}个，耗时{}ms", MAX_NUM, costTime);
            localDate = localDate.plusDays(1);
        }
    }
